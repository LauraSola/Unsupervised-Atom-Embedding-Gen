...
from train.metrics import compute_metrics_and_logging, compute_loss, compute_loss_nodes, compute_loss_adj
...
def train(model, loaders, optimizer, loggers):
    ...
    barlow_twins_loss = BarlowTwinsLoss("cuda:0",cfg.dim_in,cfg.lamda) #a crysatom tenien if cuda ... else cpu #(self,device,,embed_size,lambd):
    ...
    for cur_epoch in range(cfg.optim.max_epoch):
        ...
        embedding_dict = train_epoch(loggers[0], loaders[0], model, optimizer, cfg.batch_accumulation, scheduler, barlow_twins_loss)
        perf[0].append(loggers[0].write_epoch(cur_epoch))

        eval_epoch(loggers[1], val_fixed_data, model, barlow_twins_loss)
...
def train_epoch(logger, loader, model, optimizer, batch_accumulation, scheduler, barlow_twins_loss):
    """
    ...
    for iter, batch in tqdm(enumerate(loader), total=len(loader), ncols=50):
        ...
        if cfg.loss != "branca dalt":
            ...
            graph_embd_1, _, _ = model(batch_augment_1.clone(), graph_emb = True) 
            graph_embd_2, _, _ = model(batch_augment_2.clone(), graph_emb = True)

            graph_embd_1 = F.normalize(graph_embd_1, dim=1)
            graph_embd_2 = F.normalize(graph_embd_2, dim=1)

            batch_size = graph_embd_1.size(0)
            bt_loss = barlow_twins_loss(graph_embd_1, graph_embd_2, batch_size) 

        if cfg.loss == "combined":
            #loss = cfg.alpha * node_loss + cfg.beta * adj_loss + cfg.gamma * bt_loss
            loss = bt_loss
...
def eval_epoch(logger, loader, model, barlow_twins_loss, test_metrics=False):
    """
