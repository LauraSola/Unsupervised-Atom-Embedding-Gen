...
cros_entropy_loss = nn.CrossEntropyLoss()
...
def compute_loss_nodes(pred, true):
    
  
    true = torch.argmax(true, dim=1)  # Convert to class indices (shape: N,)
    loss  = cros_entropy_loss(pred, true)
    return loss

def compute_loss_adj(pred, true):
   
    pos_weight=torch.Tensor([0.1,1,1,1,1,1]).to("cuda:0")
    loss = F.nll_loss(pred, true,weight=pos_weight)

    return loss

def compute_loss_contrastive(pred, true):
    return contrastive(pred, true)
...
@torch.no_grad()
def compute_metrics_and_logging(pred, true, loss, lr, time_used, logger, 
                                mae = None, mse = None, node_loss = None, edge_loss = None, bt_loss = None, contrastive_loss = None, test_metrics=False,
                                u_node_loss = None, u_edge_loss = None, u_contrastive_loss = None, l_node_loss = None, l_edge_loss = None, l_contrastive_loss = None, l_MAE = None):
    """
    ...
    if test_metrics:
        ...
        if cfg.regularization_cartnet:
            logger.update_stats(true = true.to("cpu"),
                                pred = pred.to("cpu"),
                                loss = loss.mean().item(),
                                u_node_loss = u_node_loss.mean().item(),
                                u_edge_loss = u_edge_loss.mean().item(),
                                u_contrastive_loss = u_contrastive_loss.mean().item(),
                                l_node_loss = l_node_loss.mean().item(),
                                l_edge_loss = l_edge_loss.mean().item(),
                                l_contrastive_loss = l_contrastive_loss.mean().item(),
                                l_MAE = l_MAE.mean().item(),
                                lr = lr,
                                time_used = time_used,
                                params = cfg.params_count,
