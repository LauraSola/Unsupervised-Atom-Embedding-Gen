...
### PLANTEJAR-SE POSAR UNA LINEAR LAYER DESPRÉS INPUT PER TENIR EL MATEIX QUE CRYSATOM (ON TREIEM EL ENCODER I COM A ATOM ENCODER)
...
class Encoder(torch.nn.Module):
    ...
    def __init__(
        ...
        self.dim_emb = dim_in if cfg.experiment == "Copy dense vector" else dim_in*2
        self.dim_in = dim_in
        self.invariant = invariant
        self.temperature = temperature
        self.atom_types = atom_types
        if self.atom_types:
            self.embedding = nn.Embedding(119, self.dim_emb)
        ...
        if self.temperature:
            self.temperature_proj_atom = pyg_nn.Linear(1, self.dim_emb, bias=True)
        elif self.atom_types:
            self.bias = nn.Parameter(torch.zeros(self.dim_emb))
        self.activation = nn.SiLU(inplace=True)
        
        if self.temperature or self.atom_types:
            self.encoder_atom = nn.Sequential(self.activation,
                                        pyg_nn.Linear(self.dim_emb, self.dim_in),
                                        self.activation)
        if self.invariant:
            dim_edge = dim_rbf
        else:
        ...
        self.linear = pyg_nn.Linear(self.dim_in, self.dim_in)
           

    def forward(self, batch):
        if cfg.encoder != "No encoder": #NO ATOM ENCODER
            if cfg.encoder == "Linear": #LINEAR ATOM ENCODER
