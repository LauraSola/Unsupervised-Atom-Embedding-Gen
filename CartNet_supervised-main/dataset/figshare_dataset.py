...
class Figshare_Dataset(InMemoryDataset):
    ...
    def process(self):
        ...
        zero_embedding_atomic_numbers = set()
        if cfg.input_filter:
            for atom_number, embedding in embeddings_dict.items():
                if torch.all(embedding[0] == 0):  # If the embedding is zero for this atomic number
                    zero_embedding_atomic_numbers.add(atom_number)
            
            #print("atoms with zero embeddings:", zero_embedding_atomic_numbers)
        ...
        for i, (ddat, target) in tqdm(enumerate(zip(self.data, self.targets)),total=len(self.data)):
            ...
            if cfg.input_filter:
                ...
                for atom_number in atomic_numbers: 
                    if atom_number.item() in zero_embedding_atomic_numbers or atom_number.item() not in embeddings_dict.keys():
                        skip = True
            ...
            if cfg.input == "embeddings":
                emb = torch.stack([embeddings_dict[get_node_attributes(s, atom_features="atomic_number")[0]][0].clone().detach().float()
                                    for s in structure.elements
                                ])
                data = Data(x=emb, y=target)
            else:
