...
def rewrite_embeddings(model, atomic_embeddings: dict):
    """
    Initialize the embedding layer with precomputed atomic embeddings.
    """
    embedding_matrix = torch.empty((119, cfg.dim_in)) 
    nn.init.xavier_uniform_(embedding_matrix)

    for atomic_number, vector in atomic_embeddings.items():
        if atomic_number < 119:  # Ensure it fits within embedding size
            embedding_matrix[atomic_number] = torch.tensor(vector, dtype=torch.float32)
            
    embedding_matrix = embedding_matrix.to("cuda:0")
    model.encoder.embedding.weight.data = embedding_matrix  # Assign precomputed values 
...
if __name__ == "__main__":
    ...
    parser.add_argument("--experiment", type=str, default="", help = "What type of experiment of pretraining I am runing")
    parser.add_argument("--path_model", type=str, default="", help = "path of the pretrained model")
    parser.add_argument("--copiar_pesos", type=bool, default=False, help = "")
    parser.add_argument("--encoder", type=str, default="", help = "") # "No encoder" "edge encoder only"
    parser.add_argument("--input", type=str, default="", help = "") #"embeddings"
    parser.add_argument("--dense_vector", type=str, default="", help = "path of the dense vector") 
    parser.add_argument("--input_filter", type=bool, default=False, help = "") 
    ...
    cfg.copiar_pesos = args.copiar_pesos
    cfg.encoder = args.encoder
    cfg.input = args.input
    cfg.dense_vector = args.dense_vector
    cfg.input_filter = args.input_filter
    ...
    ### APROFITAR EL PRETRAINING JA FET ###

    if cfg.copiar_pesos:

        file_path = cfg.path_model

        ckpt = torch.load(file_path, weights_only=True)

        
        if cfg.experiment == "Copy dense vector":
        ...
        print("guardat", ckpt['model_state'].keys()) 
        print("actual" , model.state_dict().keys())

        # if cfg.experiment == "Freeze encoder":
        #     for param in model.encoder.parameters():  # Adjust this based on your model's structure
        #         param.requires_grad = False  # Freeze embeddings

    if cfg.experiment == "Copy dense vector" and os.path.exists(cfg.dense_vector):
        with open(cfg.dense_vector, 'rb') as f:
            atomic_embeddings = pickle.load(f)
        rewrite_embeddings(model, atomic_embeddings) 
    ...
    # if cfg.experiment == "Different lr":
    #     # Define different learning rates
    #     optimizer = torch.optim.Adam([
    #         {'params': model.encoder.parameters(), 'lr': cfg.lr/10},  # Smaller LR for encoder
    #         {'params': model.layers.parameters(), 'lr': cfg.lr},
