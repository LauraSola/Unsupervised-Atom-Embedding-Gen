...
def check_zero_embeddings(dataset, dataset_name):
    print(f"Checking zero embeddings in {dataset_name}...")
    for i, data in enumerate(dataset):
        if torch.all(data.x == 0):  # Check if the entire embedding vector is zero
            print(f"Warning: All-zero embedding found in {dataset_name} at index {i}")
...
def create_loader():
    ...
    if cfg.dataset.name == "ADP":
        ...
        radius = cfg.radius
        prefix = cfg.dataset.name+"__"+str(radius)+"_"+str(cfg.max_neighbours)+"_"+target+"_"+str(seed)
        if cfg.input == "embeddings":
            prefix = prefix + "_embd__"+str(cfg.dim_in)+str(cfg.input_filter)
    ...
    # if cfg.input == "embeddings":
    #     check_zero_embeddings(dataset_train, "train")
    #     check_zero_embeddings(dataset_val, "val")
    #     check_zero_embeddings(dataset_test, "test")

        
  
    if cfg.input == "embeddings" or cfg.input_filter:
        loaders = [
            DataLoader(dataset_train, batch_size=cfg.batch, 
                                    shuffle=True, num_workers=0),
            DataLoader(dataset_val, batch_size=cfg.batch, 
                                        shuffle=False, num_workers=0),
            DataLoader(dataset_test, batch_size=1 if cfg.dataset.name == "ADP" else cfg.batch, 
                                        shuffle=False, num_workers=0) 
        ]
    else:
        loaders = [
